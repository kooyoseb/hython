...
## Hython Manager 1.0.0 - 2026-07-27

- GitHub 릴리스 태그와 자산을 분석해 Hython 본체, Studio, VS Code 확장,
  Manager 및 새 제품을 자동으로 분류합니다.
- 설치 상태 감지, SHA-256 검증 다운로드, 설치·업데이트·제거 및 일괄
  설치·업데이트를 지원합니다.
- VS Code 확장은 `code` 명령으로 감지·설치·업데이트·제거합니다.
- 트레이 상주, 주기적 업데이트 확인, Windows 자동 시작과 설정 저장을
  지원합니다.
- 실행 중인 Manager를 안전하게 교체하는 셀프 업데이트 경로를 추가했습니다.
- 사용자 단위 MSI는 시작 메뉴와 선택적 바탕화면 바로가기, 업그레이드,
  복구 및 완전 제거를 지원합니다.

## Hython Studio 1.0.3 - 2026-07-27

- 표준입력을 사용하지 않는 실행·HBC 컴파일·EXE 빌드가
  `StandardInputEncoding` 예외로 실패하던 치명적 회귀를 수정했습니다.
- 표준입력이 실제로 리디렉션된 분석 작업에서만 UTF-8 입력 인코딩을 설정합니다.
